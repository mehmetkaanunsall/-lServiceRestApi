PrimeFaces.widget.Sentinel = PrimeFaces.widget.BaseWidget.extend({
    init: function (cfg) {
        ...
        this.menubarContainer = this.menubarontainer || this.menubar.find('ul.layout-menubar-container');
        this.subMenu = this.subMenu || this.menubar.find('li.activeSubMenu');        
        this.openSubMenu(this.subMenu.find('a'));
        ...
        {
            $('.leftMenuResizeIcon').addClass('fa-chevron-right');
            $('#layout-portlets-cover').removeClass('lp-cover1').addClass('lp-cover2');
        } else {
            $('.leftMenuResizeIcon').addClass('fa-chevron-left');
        }
    ...
    toggleLeftMenu: function () {
        if (this.menubar.hasClass('slimmenu')) {
            this.menubar.removeClass('slimmenu', 300);
            $('#searchArea').removeClass('slimsearch', 300);
            $.cookie('sentinel_menumode', 'large', {path: '/'});
            $('#layout-portlets-cover').removeClass('lp-cover2').addClass('lp-cover1', 300);
            this.openSubMenu(this.subMenu.find('a'));
        } else {
            this.menubar.addClass('slimmenu', 300).removeClass('layout-menubar-open-fullscr', 300);
            $('#searchArea').addClass('slimsearch', 300);
            $.cookie('sentinel_menumode', 'slim', {path: '/'});
             $('#layout-portlets-cover').removeClass('lp-cover1', 300).addClass('lp-cover2', 300);
        }

        this.menubar.addClass('OvHidden').scrollTop(0).removeClass('OvHidden');
        $('body').removeClass('OvHidden');
    },
    leftMenuScroll: function () {
        
        var bodyHeight = $('#layout-menubar').height();            
        if ($('#menubar').hasClass('slimmenu')) {
            bodyHeight_md = bodyHeight - 270;
        } else {
            bodyHeight_md = bodyHeight - 220;
        }

        $('.sm_defaultcompany > ul').css({"overflow-x": "none", "overflow-y": "auto", "max-height": ((bodyHeight_md / 2) + 30) + "px"}).addClass('MSScroll');
        $('.sm_modules > ul').css({"overflow-x": "none", "overflow-y": "auto", "height": bodyHeight_md + "px"}).addClass('MSScroll');               
    },
    bindEvents: function () {
        this.initTopMenu();
        ...
        $('#layout-menubar-resize').on('click', function (e) {
            ...
            $this.leftMenuScroll();
            e.preventDefault();
            $(".sm_defaultcompany > ul").css("display", "none");            
            $(".sm_modules > ul").css("display", "block");
            $('.dash_btn', 300).click();
            $('.leftMenuResizeIcon').toggleClass('right');
        });

        // responsive mode menubar open button
        $('#layout-menubar-resize2').on('click', function (e) {
            
            $('.leftMenuResizeIcon2').toggleClass('right');
            if ($this.menubar.hasClass('layout-menubar-open-fullscr')) {
                $this.menubar.removeClass('layout-menubar-open-fullscr');
                $('body').removeClass('OvHidden');
            } else {
                $('.sm_modules > ul').css({"overflow-x": "none", "overflow-y": "auto", "height": ""}).removeClass('MSScroll');
                
                $this.menubar.addClass('layout-menubar-open-fullscr').removeClass('slimmenu');
                $('#searchArea').removeClass('slimsearch');
                $('body').addClass('OvHidden');
                var bodyHeight = $('#layout-menubar').height();
                $('#sm_leftmenu > #sm_modules > ul').css({"overflow": "auto", "height": bodyHeight - 320 + "px"}).addClass('MSScroll');
            }
