...
public abstract class GeneralDefinitionBean<T> extends AuthenticationLists {

    public List<T> listOfObjects;
    public List<T> listOfFilteredObjects;
    ...
    public List<T> getListOfFilteredObjects() {
        return listOfFilteredObjects;
    }

    public void setListOfFilteredObjects(List<T> listOfFilteredObjects) {
        this.listOfFilteredObjects = listOfFilteredObjects;
    }
    ...
    public void onToggle(ToggleEvent e) {
        ...
        countToggle = 0;
        System.out.println("-----data----" + (Integer) e.getData());
        ...
        System.out.println("*-*-* " + toogleList.toString());
    ...
    public void reorder(AjaxBehaviorEvent event) {
        ...
        gridOrderColumnService.reorder(marwiz1.getPageIdOfGoToPage(), table.getClientId(), new Gson().toJson(columns));
    ...
    public void bringOrder(String gridId) {
        String jsonProperties = gridOrderColumnService.bringOrder(marwiz1.getPageIdOfGoToPage(), gridId);

        Gson gson = new Gson();
        try {
            columns = gson.fromJson(jsonProperties, new TypeToken<ArrayList<DataTableColumn>>() {
            }.getType());
    ...
    public void updateGridProperties(String gridId) {
        ...
        if (columns.isEmpty()) {
            ...
            if ("frmTank:dtbTank".equals(gridId) || "frmBankAccount:dtbBankAccount".equals(gridId) || "frmAccount:dtbAccount".equals(gridId)) {
                ...
                for (UIComponent column : table.getColumnGroup("footer").getChildren()) {
                    //  System.out.println("Column Group " + column.getClientId());
                    Column newcolumn = (Column) column;
                    for (DataTableColumn column1 : columns) {
    ...
    public void waitToExportExcel() throws InterruptedException {
        try {
            System.out.println("****waitToExportExcel");
            Double d = (double) listOfObjects.size() / 2;
    ...
    public void waitToExportPdf() throws InterruptedException {
        try {
            Double d = (double) listOfObjects.size() / 3;
            TimeUnit.MILLISECONDS.sleep(d.intValue());
        } catch (Exception e) {
        }
