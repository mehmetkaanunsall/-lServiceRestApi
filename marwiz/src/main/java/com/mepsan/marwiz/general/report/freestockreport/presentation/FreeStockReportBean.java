 ...
 *
 * @date   25.03.2020 04:15:39
 */
package com.mepsan.marwiz.general.report.freestockreport.presentation;
...
import com.mepsan.marwiz.general.report.freestockreport.business.IFreeStockReportService;
...
@ManagedBean
@ViewScoped
public class FreeStockReportBean extends GeneralReportBean<FreeStockReport> {

    @ManagedProperty(value = "#{freeStockReportService}")
    public IFreeStockReportService freeStockReportService;
    ...
    public void setFreeStockReportService(IFreeStockReportService freeStockReportService) {
        this.freeStockReportService = freeStockReportService;
    }
    ...
    @PostConstruct
    public void init() {
        System.out.println("------------FreeStockReportBean");
        listOfStock = new ArrayList<>();
        listOfCategorization = new ArrayList<>();
        selectedObject = new FreeStockReport();
        ...
        if (sessionBean.getUser().getLastBranchSetting().isIsCentralIntegration() && sessionBean.getUser().getLastBranch().getConceptType() == 1) {
            ...
            toogleList = Arrays.asList(true, true, true, true, true, true, true, false, true, true, true, true, true);
        }
    ...
    public void find() {
        ...
        createWhere = freeStockReportService.createWhere(selectedObject, isCentralSupplier, centralSupplierBookCheckboxFilterBean.getSupplierType());
        DataTable dataTable = (DataTable) FacesContext.getCurrentInstance().getViewRoot().findComponent("frmFreeStockReportDatatable:dtbFreeStockReport");
        if (dataTable != null) {
    ...
    @Override
    public LazyDataModel<FreeStockReport> findall(String where) {
        return new CentrowizLazyDataModel<FreeStockReport>() {
            @Override
            public List<FreeStockReport> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
                List<FreeStockReport> result = freeStockReportService.findAll(first, pageSize, sortField, convertSortOrder(sortOrder), filters, where, branchList);
                int count = freeStockReportService.count(where, branchList);
    ...
    public void updateAllInformation(ActionEvent event) {
        if (event.getComponent().getParent().getParent().getId().equals("frmStockBookFilterCheckbox")) {
            ...
            RequestContext.getCurrentInstance().update("frmFreeStockReport:txtStock");
            ...
            RequestContext.getCurrentInstance().update("frmFreeStockReport:txtCentralSupplier");
            ...
            RequestContext.getCurrentInstance().update("frmFreeStockReport:txtSupplier");
        } else {
            ...
            RequestContext.getCurrentInstance().update("frmFreeStockReport:txtCategory");
        }
    ...
    public void createPdf() {
        freeStockReportService.exportPdf(createWhere, selectedObject, toogleList, branchList, selectedBranchList, isCentralSupplier);
    }

    public void createExcel() throws IOException {
        freeStockReportService.exportExcel(createWhere, selectedObject, toogleList, branchList, selectedBranchList, isCentralSupplier);
    }

    public void createPrinter() {
        RequestContext.getCurrentInstance().execute("$(\"#printerPanel\").empty();$(\"#printerPanel\").append('" + StaticMethods.escapeStringForHtml(freeStockReportService.exportPrinter(createWhere, selectedObject, toogleList, branchList, selectedBranchList, isCentralSupplier)) + "');$(\"#printerPanel\").css('display','block');print_page();$(\"#printerPanel\").css('display','none');");
